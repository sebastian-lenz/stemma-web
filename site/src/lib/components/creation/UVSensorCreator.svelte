<script lang="ts">
  ...
  async function create() {
    creating = true;
    try {
      const device = appState.manager!.getUVSensor();
      await device.connect();
      appState.addInstance({ kind: "uv-sensor", key: "uv-sensor:0x53", device });
      oncreated();
    } finally {
      creating = false;
    }
...
<div class="space-y-3">
  <p class="text-xs text-gray-400">Fixed I2C address: 0x53</p>
