<script lang="ts">
  ...
  import { TouchSensorDevice } from "stemma-web";
  import type { TouchSensorAddress } from "stemma-web";

  let { oncreated }: { oncreated: () => void } = $props();

  let address = $state<TouchSensorAddress>(TouchSensorDevice.ADDRESSES[0]);
...
<div class="space-y-3">
  <label class="block space-y-1">
    ...
    >
      {#each TouchSensorDevice.ADDRESSES as addr}
