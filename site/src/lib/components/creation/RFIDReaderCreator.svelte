<script lang="ts">
  ...
  import { RFIDReaderDevice } from "stemma-web";
  import type { RFIDReaderAddress } from "stemma-web";

  let { oncreated }: { oncreated: () => void } = $props();

  let address = $state<RFIDReaderAddress>(RFIDReaderDevice.ADDRESSES[0]);
  let creating = $state(false);

  async function create() {
    creating = true;
    try {
      const device = appState.manager!.getRFIDReader(address);
      await device.connect();
      const key = `rfid-reader:${address}`;
      appState.addInstance({ kind: "rfid-reader", key, device });
      oncreated();
    } finally {
      creating = false;
    }
...
<div class="space-y-3">
  <label class="block space-y-1">
    ...
    >
      {#each RFIDReaderDevice.ADDRESSES as addr}
